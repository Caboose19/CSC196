...
#include "../Game.h"
	...
	bool DeathStar::Load(const std::string& filename)
	{
		bool success = false;
	...
	void DeathStar::Update(float dt)
	{
		...
		nc::Vector2 DeathStarVelocity = direction.Noramlized() * m_thrust;
		m_transform.position = m_transform.position + DeathStarVelocity * dt;

		m_transform.Update();

	}

	void DeathStar::OnCollision(Actor* actor)
	{
